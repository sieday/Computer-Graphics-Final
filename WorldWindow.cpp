...
WorldWindow::draw(void)
{
	...
	teacups.Initialize(); //EEPER 11/9/22
	trees.Initialize();
	box.Initialize();
    }
	...
	else
	{
		...
		if (cameraMode == 2)
		{
			mult = 8;
			//Normalize_3(deriv);
			float hold = (sqrt(deriv[0] * deriv[0] * deriv[1] * deriv[1] * deriv[2] * deriv[2]));
			if(hold != 0)
			{
				deriv[0] /= hold;
				deriv[1] /= hold;
		...
		glMatrixMode(GL_MODELVIEW);
		glLoadIdentity();
		gluLookAt(posn[0] - (mult * deriv[0]), posn[1] - (mult * deriv[1]), posn[2] - (mult * deriv[2]), posn[0] + deriv[0], posn[1] + deriv[1], posn[2] + deriv[2], 0.0, 0.0, 1.0);
...
// space. This is a directional light - note the 0 in the w component.
	...
	ground.Draw();
	traintrack.Draw(); 
	teacups.Draw(); //EEPER 11/9/2022
	trees.Draw();
	path.Draw();
	box.Draw();

	//MODIFIDCATION END - EEPER
